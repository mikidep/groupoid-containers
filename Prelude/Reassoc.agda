...
data Term {ℓ : Level} : {A : Type ℓ} → A → A → Typeω where
  refl′ : ∀ {A} {x : A} → Term x x
  ↑_ : ∀ {A} {x y : A} (p : x ≡ y) → Term x y
  _∙′_ : ∀ {A} {x y z : A} → Term x y → Term y z → Term x z
  cong′ : ∀ {ℓ'} {A} {B : Type ℓ'} {x y : B}  
    (f : B → A)
    → Term x y
    → Term (f x) (f y)
...
data Ev {A : Type ℓ} : (x y : A) → Type (ℓ-suc ℓ) where
  nil : ∀ {x : A} → Ev x x
  _∷_ : {x y z : A} 
        (p : x ≡ y) 
        (l : Ev y z) 
...
ev-cong : {A : Type ℓ} {B : Type ℓ'} {x y : A}
