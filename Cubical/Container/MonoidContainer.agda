...
module _ (pmc : PsMndCont) where
  ...
  PsMndCont→Pseudomonoid .assoc-coh = 
    Square→compPath 
      (CMor□′ λ ss → ΣSquare (
        -- subst (λ x → Square _ x _ _) (sym (congFunct fst ? ?)) 
        (let
          s = ss .fst .fst .fst
          s′ = ss .fst .fst .snd
          s″ = curry (ss .fst .snd)
    ...
    --   s‴ = curry (curry s‴')
    --   -- assoc-σ {s = s} {s′ = s′} 
    --   --     {s″ = λ p q → m (s″ p q) (s‴ p q)}
    --   -- ∙ assoc-σ {s = m s s′} {s′ = m↖↗ s″} 
    --   --     {s″ = λ p → s‴ (↖ p) (↗ p)}
    --   a = assoc-σ {s = s} {s′ = s′} {s″ = λ p q → m (s″ p q) (s‴ p q)}
    --   b = λ i → m (assoc-σ i) (λ p → s‴ (assoc-π₁ i p) (assoc-π₂ i p) (assoc-π₃ i p))
    --   c = assoc-σ {s = m s s′} {s′ = m↖↗ s″} {s″ = λ p → s‴ (↖ p) (↗ p)}
    --   goal : Square
