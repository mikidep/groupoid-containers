...
module _ {𝒰 : Universe ℓc ℓe} {K : Universe.Code 𝒰 → Type ℓk} (O : Operad 𝒰 K) where
  ...
  -- The auxiliary path underlying `join-assoc` (Theorem 8.2, associativity
  -- of join), with its components named so that downstream coherence (the
  -- monad 2-cell pentagon, `Monad.TwoCellCoherence`) can reason about the
  -- `Index`/`Data` of `join-assoc`. `join-assoc O (I ▷ k ▷ D)` is
  -- `sym (JoinAssocAux.aux I k D)`. Natural direction of `⅀Assoc≃` /
  -- `Operad.assoc` is nested-right → nested-left.
  module JoinAssocAux {X : Type ℓx} (I : Code) (k : K I)
                      (D : El I → OpM O (OpM O X)) where
    -- Destructure the argument two levels deep.
    J : El I → Code
    J a = Index (D a)
    ks : (a : El I) → K (J a)
    ks a = Op (D a)
    ...
    -- B' : the second-component family of the target Σ in
    -- Σ-cong-equiv-fst (⟦⅀⟧ I J). B' (a , b) = El (M a b).
    B' : Σ[ a ∈ El I ] El (J a) → Type ℓe
    B' ab-pair = El (M (fst ab-pair) (snd ab-pair))

    -- The homogeneous pointwise equation. Computing equivFun (⅀Assoc≃ I J M)
    -- abc (a , bc) → (a , (b , c)) → ((a , b) , c) → (ab* , c-bd) →
    -- invEq e₄ (ab* , c-bd) where ab* = invEq (⟦⅀⟧ I J) (a , b) and c-bd is
    -- a subst of c. Then data-NL of this unfolds to
    -- F (fst (equivFun (⟦⅀⟧ I J) α)) (snd …) γ with
    -- (α , γ) = equivFun e₄ (invEq e₄ (ab* , c-bd)). Two secEq insertions
    ...
    homog-pt abc =
      let aBc : Σ[ a ∈ El I ] El (⅀ (J a) (M a))
          ...
          -- step2 slides (ab* , c-bd) along
          -- sym (secEq (⟦⅀⟧ (⅀ I J) …) (ab* , c-bd)). Type-correct without
          -- extra transports because (snd q : El (⅀Assoc-C' I J M (fst q)))
  ...
  opaque
    ...
    join-assoc {X = X} (I ▷ k ▷ D) = sym (JoinAssocAux.aux I k D)

    -- Exposes `join-assoc`'s definition for downstream coherence proofs
    -- without requiring them to `unfolding join-assoc`.
    join-assoc≡sym-aux
      : {X : Type ℓx} (I : Code) (k : K I) (D : El I → OpM O (OpM O X))
      → join-assoc {X = X} (I ▷ k ▷ D) ≡ sym (JoinAssocAux.aux {X = X} I k D)
    join-assoc≡sym-aux I k D = refl
