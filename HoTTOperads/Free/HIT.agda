...
open import HoTTOperads.Universe.Assoc
  using ( adj-coh ; ⅀-subst-path ; transp-⅀IdlD ; transp-⅀-subst-path
        ; ⅀-subst-path-equiv ; Assoc-cont ; Assoc-cont-at-pair
...
module _ {𝒰 : Universe ℓc ℓe} where
  ...
  -- §4  Substitution toolkit
  --
  -- Two lemmas for pushing a `subst (FreeOps K) _` across an outer `graft`:
  --   * `graft-subst-fst`  pushes the subst over the first (tree) argument,
  --     reindexing along `⅀-subst-path` (HoTTOperads.Universe.Assoc).
  ...
  graft-assoc K _ B C leaf ts tss = toPathP eq-leaf
    where
      ...
      pair-eq b = cong (equivFun (⟦⅀⟧ 𝜏 B)) (transp-⅀IdlD 𝒰 B b)
                ∙ secEq (⟦⅀⟧ 𝜏 B) (α , b)
      ...
      opaque
        funExt-q-decomp : (b₀ : El (B α))
                        ...
                        ∙ cong (⅀Assoc-C' 𝜏 B C) (sym (transp-⅀IdlD 𝒰 B b₀))
        funExt-q-decomp b₀ =
            cong (cong C-uncurry)
                 (symDistr (cong (equivFun (⟦⅀⟧ 𝜏 B)) (transp-⅀IdlD 𝒰 B b₀))
                           (secEq (⟦⅀⟧ 𝜏 B) (α , b₀)))
          ∙ congFunct C-uncurry
                      (sym (secEq (⟦⅀⟧ 𝜏 B) (α , b₀)))
                      (cong (equivFun (⟦⅀⟧ 𝜏 B)) (sym (transp-⅀IdlD 𝒰 B b₀)))
      ...
      opaque
        snd-PathP : (x : El (⅀ (B α) (C α)))
                  → PathP (λ i → El (⅀Assoc-C' 𝜏 B C
                                       (sym (transp-⅀IdlD 𝒰 B (fst (equivFun (⟦⅀⟧ (B α) (C α)) x))) i)))
                          (c₀'-of x)
        ...
        σ-bridge : (x : El (⅀ (B α) (C α)))
                 → Path (Σ (El (⅀ 𝜏 B)) (λ ab → El (⅀Assoc-C' 𝜏 B C ab)))
                        ...
                        (transp-B (fst (equivFun (⟦⅀⟧ (B α) (C α)) x))
                          , transport (cong El (funExt⁻ q (fst (equivFun (⟦⅀⟧ (B α) (C α)) x))))
                                      (snd (equivFun (⟦⅀⟧ (B α) (C α)) x)))
        σ-bridge x = ΣPathP (sym (transp-⅀IdlD 𝒰 B (fst (equivFun (⟦⅀⟧ (B α) (C α)) x)))
                            , snd-PathP x)
      ...
      opaque
        ...
        LHS-chain x =
          ...
          ∙ cong (transport (cong El (Inj (⅀Assoc≃ 𝜏 B C)))) (transp-⅀IdlD 𝒰 D₀ x)
          ∙ step-Assoc-on-pair 𝒰 𝜏 B C (α , x)
      ...
      opaque
        RHS-chain : (x : El (⅀ (B α) (C α)))
                  → transport (cong El (cong (⅀ (B α)) q
                                       ∙ ⅀-subst-path 𝒰 (⅀IdlD 𝒰 B) (⅀Assoc-C' 𝜏 B C))) x
                  ≡ invEq (⟦⅀⟧ (⅀ 𝜏 B) (⅀Assoc-C' 𝜏 B C))
                          (transp-B (fst (equivFun (⟦⅀⟧ (B α) (C α)) x))
                          , transport (cong El (funExt⁻ q (fst (equivFun (⟦⅀⟧ (B α) (C α)) x))))
                                      (snd (equivFun (⟦⅀⟧ (B α) (C α)) x)))
        RHS-chain x =
            cong (λ p → transport p x)
                 (congFunct El (cong (⅀ (B α)) q)
                              (⅀-subst-path 𝒰 (⅀IdlD 𝒰 B) (⅀Assoc-C' 𝜏 B C)))
          ∙ substComposite (λ X → X)
                           (cong El (cong (⅀ (B α)) q))
                           (cong El (⅀-subst-path 𝒰 (⅀IdlD 𝒰 B) (⅀Assoc-C' 𝜏 B C))) x
          ∙ cong (transport (cong El (⅀-subst-path 𝒰 (⅀IdlD 𝒰 B) (⅀Assoc-C' 𝜏 B C))))
                 transport-q-x
          ∙ transp-⅀-subst-path 𝒰 (⅀IdlD 𝒰 B) (⅀Assoc-C' 𝜏 B C)
      ...
      opaque
        bridge : ⅀IdlD 𝒰 D₀ ∙ Inj (⅀Assoc≃ 𝜏 B C)
               ≡ cong (⅀ (B α)) q ∙ ⅀-subst-path 𝒰 (⅀IdlD 𝒰 B) (⅀Assoc-C' 𝜏 B C)
        bridge =
            sym (InjSec 𝒰 (⅀IdlD 𝒰 D₀ ∙ Inj (⅀Assoc≃ 𝜏 B C)))
          ∙ cong Inj equivs-agree
          ∙ InjSec 𝒰 (cong (⅀ (B α)) q ∙ ⅀-subst-path 𝒰 (⅀IdlD 𝒰 B) (⅀Assoc-C' 𝜏 B C))
      ...
      eq-leaf =
        ...
        ∙ substComposite (FreeOps K)
                         ...
                         (⅀-subst-path 𝒰 (⅀IdlD 𝒰 B) (⅀Assoc-C' 𝜏 B C))
                         inner-graft
        ∙ cong (subst (FreeOps K) (⅀-subst-path 𝒰 (⅀IdlD 𝒰 B) (⅀Assoc-C' 𝜏 B C)))
  ...
  graft-assoc K _ B C (node A' B' k ts') ts tss = toPathP eq-node
    where
      ...
      RHS-path-tail = ⅀AssocD 𝒰 A' B'' C1
                    ∙ ⅀-subst-path 𝒰 (⅀AssocD 𝒰 A' B' B) (⅀Assoc-C' (⅀ A' B') B C)
      ...
      opaque
        ...
        RHS-chain-on-pair a z =
          ...
          ∙ substComposite (λ X → X)
                           ...
                           (cong El (⅀-subst-path 𝒰 (⅀AssocD 𝒰 A' B' B) (⅀Assoc-C' (⅀ A' B') B C)))
                           (invEq (⟦⅀⟧ A' B-RHS) (a , transport (cong El (per-fibre-Δ a)) z))
          ∙ cong (transport (cong El (⅀-subst-path 𝒰 (⅀AssocD 𝒰 A' B' B) (⅀Assoc-C' (⅀ A' B') B C))))
                 (transp-⅀AssocD-RHS-on-pair a (transport (cong El (per-fibre-Δ a)) z))
          ∙ transp-⅀-subst-path 𝒰 (⅀AssocD 𝒰 A' B' B) (⅀Assoc-C' (⅀ A' B') B C)
                                 (invEq (⟦⅀⟧ (⅀ A' B'') C1)
