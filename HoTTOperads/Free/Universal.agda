  ...
  module _ where
    ...
    graft-eta A B k ts =
      ...
      where
        ...
        --     `transp-⅀IdlD`.
        bridge : ⅀AssocD 𝒰 A (λ _ → 𝜏) B-tp ∙ ⅀-subst-path 𝒰 p B
               ≡ cong (⅀ A) B-shape≡B
        bridge =
          ...
          ∙ InjSec 𝒰 rhs-path
          where
            lhs-path = ⅀AssocD 𝒰 A (λ _ → 𝜏) B-tp ∙ ⅀-subst-path 𝒰 p B
            ...
            LHS-chain a w =
              ...
              ∙ substComposite (λ X → X)
                               ...
                               (cong El (⅀-subst-path 𝒰 p B))
                               (invEq (⟦⅀⟧ A B-shape) (a , w))
              ∙ cong (transport (cong El (⅀-subst-path 𝒰 p B)))
                     (transp-⅀AssocD-pair 𝒰 A (λ _ → 𝜏) B-tp a w)
              ∙ transp-⅀-subst-path 𝒰 p B
                                     (invEq (⟦⅀⟧ (⅀ A (λ _ → 𝜏)) B-tp)
                ...
                v₀-shifted-α-via-w =
                  ...
                  where
                    ...
                    w-eq = sym (retEq (⟦⅀⟧ 𝜏 D) w)
                         ...
                         ∙ sym (transp-⅀IdlD 𝒰 D v₀-shifted-α)
